...
{
	public class OverrideActionStateTreeView : TreeView
	{
		...
		ActionSubState signalAssetContext { get; set; }
		public bool readonlySignals { get; set; }

		const float k_VerticalPadding = 5;
		const float k_HorizontalPadding = 5;

		public OverrideActionStateTreeView(TreeViewState state, MultiColumnHeader multiColumnHeader, ActionStateController receiver, SerializedObject serializedObject)
		...
		public void SetSignalContext(ActionSubState assetContext = null)
		{
		...
		{
			signals = serializedObject.FindProperty("stateOverrides");
			eventsPositive = serializedObject.FindProperty("onEnterStateOverride");
			eventsNegative = serializedObject.FindProperty("onExitStateOverride");
			Reload();
		}
		...
		float GetTotalHeight()
		{
			...
			{
				var signalListItem = item as OverrideActionStateItem;
		...
		{
			var item = (OverrideActionStateItem)args.item;
		...
		{
			var item = treeItem as OverrideActionStateItem;
		...
		void AddItem(TreeViewItem root, int id, bool enabled = true)
		{
			...
			root.children.Add(new OverrideActionStateItem(signal, evt, evtNeg, id, readonlySignals, enabled, this));
		}

		static int FindIdForSignal(SerializedProperty signals, ActionSubState signalToFind)
		{
			for (var i = 0; i < signals.arraySize; ++i)
			{
