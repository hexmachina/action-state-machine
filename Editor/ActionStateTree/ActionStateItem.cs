...
{
	public class ActionStateItem : TreeViewItem
	{
		...
		readonly ActionStateTreeView m_TreeView;

		//ActionState signalAsset { get; set; }
		public ActionState signalAsset
		{
			get { return m_CurrentReceiver.GetStateAssetAtIndex(m_CurrentRowIdx); }
			set
			{
				Undo.RegisterCompleteObjectUndo(m_CurrentReceiver, L10n.Tr("Create New Signal Asset"));
				m_CurrentReceiver.ChangeStateAtIndex(m_CurrentRowIdx, value);
			}
		}
		...
		public ActionStateItem(SerializedProperty signalAsset, SerializedProperty eventPositive, SerializedProperty eventNegative, int id, bool readonlySignal, bool enabled, ActionStateTreeView treeView)
			: base(id, 0)
		{
			m_Asset = signalAsset;
			m_Evt = eventPositive;
		...
		IEnumerable<ActionState> AvailableSignalAssets(ActionState signalAsset)
		{
			var ret = ActionStateManager.assets.Except(m_CurrentReceiver.GetRegisteredStates());
			return signalAsset == null ? ret : ret.Union(new List<ActionState> { signalAsset }).ToList();
		}

		void CreateNewSignalAsset(string path)
		{
			var newSignalAsset = ActionStateManager.CreateSignalAssetInstance(path);
			m_CurrentReceiver.ChangeStateAtIndex(m_CurrentRowIdx, newSignalAsset);
		}
	}
}