...
{
	public class ActionSignalItem : TreeViewItem
	{
		...
		readonly ActionSignalTreeView m_TreeView;

		//ActionSignal signalAsset { get; set; }
		public ActionSignal signalAsset
		{
			...
			set
			{
				...
				//EditorUtility.SetDirty(m_CurrentReceiver);
			}
		}
		...
		public ActionSignalItem(SerializedProperty signalAsset, SerializedProperty eventPositive, SerializedProperty eventNegative, int id, bool readonlySignal, bool enabled, ActionSignalTreeView treeView)
			: base(id, 0)
		{
			m_Asset = signalAsset;
			m_Evt = eventPositive;
		...
		void DrawReactionColumn(Rect rect, int rowIdx)
		{
			...
			using (new EditorGUI.DisabledScope(!enabled))
			{
				...
				{
					EditorGUI.PropertyField(rect, m_Evt, new GUIContent("On Positive"));
				...
				using (var change = new EditorGUI.ChangeCheckScope())
				{
					...
					EditorGUI.PropertyField(rect, m_EvtNeg, new GUIContent("On Negative"));
					//EditorGUI.PropertyField(rect, m_Evt);
					if (change.changed)
						m_TreeView.dirty = true;
				}
		...
		IEnumerable<ActionSignal> AvailableSignalAssets(ActionSignal signalAsset)
		{
			var ret = ActionSignalManager.assets.Except(m_CurrentReceiver.GetRegisteredSignals());
			return signalAsset == null ? ret : ret.Union(new List<ActionSignal> { signalAsset }).ToList();
		}

		void CreateNewSignalAsset(string path)
		{
			var newSignalAsset = ActionSignalManager.CreateSignalAssetInstance(path);
